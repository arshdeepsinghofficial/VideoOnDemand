...
public class ViewAllMovies2 extends javax.swing.JFrame
{
   public ArrayList<Movies> alData=new ArrayList<>();
    public ViewAllMovies2(ArrayList<Movies>al)
    {
        initComponents();
        alData=al;
    ...
    public static void main(String args[])
    {
        ...
        java.awt.EventQueue.invokeLater(new Runnable()
        {
            public void run()
            {
               // new ViewAllMovies2().setVisible(true);
            }
        });
    }
        ...
        public void run()
        {
            try
            {
                ...
                for (int i = 0; i < algenre.size(); i++)
                {
                   ...
                   // ArrayList<Movies> alData = new ArrayList<>();
                    ...
                    while (true)
                    {
                        ...
                        } else if (inpu.equals("receive"))
                        {
                            ...
                            {
                                Logger.getLogger(ViewAllMovies2.class.getName()).log(Level.SEVERE, null, ex);
                            }
...
//                            alData.add(new Movies(cname, genre, producer, director, storyline, releasedate, id, title, duration,
//                                    rating, language, coverphoto, cast, squarephoto, videopath, trailerpath));
        ...
        private void showPanels(String genre, ArrayList<Movies> al)
        {
            try
            {
                int x = 10;
                int y = 10;
                GeneredVideo generedVideos = new GeneredVideo();
                int height = (al.size() * 270) + (al.size() * 10);
                generedVideos.jpall.setPreferredSize(new Dimension(getWidth(), height));
                generedVideos.jpane.setSize(new Dimension(getWidth(), generedVideos.jpane.getHeight()));
                generedVideos.lbGenre.setText(genre);
                generedVideos.setBounds(this.x, this.y, getWidth(), 265);
                this.y = this.y + 280;
                jpAllMovies.add(generedVideos);
                for (int i = 0; i < al.size(); i++)
                {
                    Movies videoData = al.get(i);
                    SingleMovie image = new SingleMovie(videoData);
                    String filename1 = videoData.coverphoto;
                    ...
                    image.lbmovies.setIcon(new ImageIcon(scaledInstance1));
                    image.lbmovies.addMouseListener(new MouseAdapter()
                    {
                        @Override
                        public void mouseClicked(MouseEvent e)
                        {
                            System.out.println(videoData.videopath);
                            LaunchMovies launchMovie = new LaunchMovies(videoData);
                            launchMovie.setVisible(true);
                        }
                    });
                    generedVideos.jpall.add(image);
                    generedVideos.jpall.repaint();
                    if ((i + 1) % 3 == 0)
                    {
                        x = 10;
                        y = y + 150 + 10;
